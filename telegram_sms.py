...
SMS_TEMPLATE       = os.environ.get("SMS_TEMPLATE", "Telegram [{chat}] {sender}: {message}")
MAX_MSG_LENGTH     = int(os.environ.get("MAX_MSG_LENGTH", "120"))

def parse_watched_chats(raw):
    if not raw: return []
    result = []
    for entry in raw.split(","):
        entry = entry.strip()
        if not entry: continue
        try: result.append(int(entry))
        except ValueError: result.append(entry.lower())
    return result

def should_notify(chat_id, chat_name, watched):
    if not watched: return True
    for entry in watched:
        if isinstance(entry, int) and entry == chat_id: return True
        if isinstance(entry, str) and entry == chat_name.lower(): return True
    return False

def send_sms(sender, chat, message):
    try:
        if len(message) > MAX_MSG_LENGTH: message = message[:MAX_MSG_LENGTH] + "..."
...
async def handle_message(update, context):
    try:
        ...
        if not should_notify(chat_id, chat_name, watched): return
        text = message.text or message.caption or "[Kein Text]"
...
def main():
    watched = parse_watched_chats(WATCHED_CHATS_RAW)
    watch_info = ", ".join(str(w) for w in watched) if watched else "alle Chats"
